    ...
    <div className="min-h-[80vh] flex items-center justify-center">
      <div className="w-full max-w-md mx-auto px-4">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Acceso al Portal</h2>
          <p className="text-gray-500 mt-2 text-sm">
            Ingresa tu código de acceso para ver los documentos de tu operación
          </p>
        </div>
        ...
        <div className={`${shaking ? "animate-shake" : ""}`}>
          <input
            ...
            maxLength={6}
            className="w-full text-center text-2xl font-mono tracking-[0.5em] py-4 px-6 
              border-2 border-gray-200 rounded-xl focus:border-[#1e3a5f] focus:ring-2 
              focus:ring-[#1e3a5f]/20 outline-none transition-all bg-white
              placeholder:tracking-normal placeholder:text-base placeholder:text-gray-300"
            autoFocus
          />
          ...
          <button
            ...
            className="w-full mt-4 py-3.5 bg-[#1e3a5f] text-white rounded-xl font-medium 
              text-sm disabled:opacity-30 hover:bg-[#2a4d7a] active:bg-[#162d4a] 
              transition-all shadow-lg shadow-[#1e3a5f]/20"
          >
            Acceder
          </button>
        </div>
        ...
        <p className="text-center text-xs text-gray-400 mt-8">
          Contacta a tu asesor si no tienes tu código
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-3 py-3 px-4 border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors">
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0
        ${hasFile ? "bg-emerald-100 text-emerald-600" : "bg-red-50 text-red-400"}`}>
        {hasFile ? "✓" : "○"}
      </div>
      <div className="flex-1 min-w-0">
        <p className={`text-sm ${hasFile ? "text-gray-500" : "text-gray-900"}`}>
          {doc.nombre_doc}
        </p>
        {doc.fecha_subida && (
      ...
      {hasFile ? (
        <button className="text-xs text-gray-500 hover:text-[#1e3a5f] px-3 py-1.5 rounded-lg border border-gray-200 hover:border-[#1e3a5f] transition-colors">
          Ver
        </button>
      ) : (
        <button
          onClick={() => onUpload(doc.id)}
          className="text-xs bg-[#1e3a5f] hover:bg-[#2a4d7a] text-white px-3 py-1.5 rounded-lg transition-colors shadow-sm"
        >
          Subir
        </button>
      )}
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-3 overflow-hidden">
      ...
      >
        ...
        <span className={`text-gray-400 transition-transform text-xs ${open ? "rotate-180" : ""}`}>
          ▼
        </span>
      </button>
      {open && (
...
/* ─── Operation Dashboard (single cierre) ─── */
      ...
      <div className="relative rounded-2xl overflow-hidden mb-6 shadow-lg">
        {operation.imagen_fondo ? (
          <div
            ...
            style={{ backgroundImage: `url(${operation.imagen_fondo})` }}
          />
        ) : (
        ...
        <div className="relative z-10 px-6 py-10 sm:px-8 sm:py-14">
          ...
          <div className="max-w-lg">
            <span className="text-xs font-medium text-white/60 uppercase tracking-wider">
              {OPERATION_LABELS[operation.tipo]}
            </span>
            <h1 className="text-3xl sm:text-4xl font-bold text-white mt-2 leading-tight">
              {operation.nombre}
            </h1>
            <p className="text-white/60 text-sm mt-2">
              {new Date(operation.fecha_creacion).toLocaleDateString("es-MX", {
                year: "numeric",
                month: "long",
                day: "numeric",
              })}
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{operations.length} operaciones activas</p>
        </div>
        <button
          onClick={onLogout}
          className="text-xs text-gray-500 hover:text-gray-700 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg transition-colors"
        >
          Salir
        </button>
      </div>
              ...
              <div className="relative h-32 overflow-hidden">
                {op.imagen_fondo ? (
                  <div
                    ...
                    style={{ backgroundImage: `url(${op.imagen_fondo})` }}
                  />
                ) : (
                  <div className="absolute inset-0 bg-gradient-to-br from-[#1e3a5f] to-[#3b82f6]" />
                )}
                ...
                <div className="absolute bottom-3 left-4 right-4">
                  ...
                  <p className="text-white/60 text-xs">{OPERATION_LABELS[op.tipo]}</p>
                </div>
                <span className={`absolute top-3 right-3 text-xs px-2 py-0.5 rounded-full font-medium backdrop-blur-sm
                  ${op.status === "activa" ? "bg-emerald-500/80 text-white" : "bg-gray-500/80 text-white"}`}>
                  {op.status === "activa" ? "Activa" : "Cerrada"}
                </span>
              </div>
...
export default function Home() {
  ...
  if (view === "pin") return <PinEntry onAccess={handleAccess} />;
  if (view === "admin") return <AdminPanel onSelect={(op) => { setActiveOp(op); setView("operation"); }} onLogout={handleLogout} />;
  if (view === "operation" && activeOp) return <OperationDashboard operation={activeOp} onLogout={handleLogout} />;
